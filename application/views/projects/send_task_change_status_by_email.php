                          ...
                          <table cellpadding="0" cellspacing="0" border="0">
                              <tbody><tr>
                                    <td width="57" valign="top">
                                      <?php
                                      $url_picture = '.'.$this->config->item('uri_images_users').'/'.$task['user_id'].'_thumb_small.jpg';
                                    ...
                                    <td style="padding-left:15px;font-size:14px;font-family:Helvetica,Arial,sans-serif" valign="top"><div class="im">
                                          <h1 style="line-height:1.3em;font-size:14px;margin:0 0 15px 0;font-family:Helvetica,Arial,sans-serif;font-weight:normal">
                                             <?php                         
                                             $changer_show_name ="";

                                             if(isset($task['changer_name']))
                                             {
                                                $changer_show_name = $task['changer_name']." ".$task['changer_last_name'];                     
                                             }
                                             else
                                             {
                                                $changer_show_name = $task['changer_email'];
                                             }
                                             $url_profile_user = site_url("/auth/edit_profile?changer_id=".$task['user_id']);
                                             ?>
                                             
                                                <?php echo $changer_show_name;?>
                                             
                                             <?php echo ", ".$view_labels['task_change_status']?>: <?php echo $view_labels['action_status'][$status]?>
                                             <br>
                                             <span style="font-weight:bold">
                                                ...
                                                $url_view_comment = site_url("pm/view_comment/?comment_id=".$parent_id."&project_id=".$project_id."#item_task_".$task['id_object']);                                                   
                                    ...
                                    <div style="line-height:1.3em">
                                       ...
                                       if(isset($task['name']))
                                       {
                                          echo nl2br(decode_chars_special($task['name']));
                                       }
                                       else
                                       {
                                          echo nl2br(decode_chars_special($task['description']));
                                       }
                                       ?>
                                    </div>
